...
const properties = require('tea-properties');
const setPathValue = properties.set;
const getPathValue = properties.get;
const mixin = {};
...
mixin.stubWithReturn = function stubWithReturn(config) {
  ...
  if (config.spies) {
    ...
    if (is.string(config.spies) && /\./.test(config.spies)) {
      ...
      for (const s = 0; s < spies.length; s++) {
...
mixin.doubleMany = function doubleMany(type, obj, methods) {
  ...
  for (const m = 0; m < methods.length; m++) {
